...
export function Account() {
  ...
  useEffect(() => {
    if (enabled !== false || !account || account.rows > 0) return
    void (async () => {
  ...
  const deciding = session?.signedIn && enabled === false && account && account.rows > 0
    ...
    <div className="goals">
      ...
      {session?.signedIn && (
        <section className="card pad">
          ...
          <p className="hint">Puedes llevarte lo que guarda el servidor, vaciar este móvil, o borrar la cuenta entera.</p>
          <div className="footer-row">
