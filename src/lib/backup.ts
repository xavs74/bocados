...
export async function exportBackup(): Promise<void> {
  const data = {
    ...
    planned: await db.planned.toArray(),
...
export async function importBackup(file: File): Promise<{ foods: number; entries: number }> {
  ...
  await db.transaction('rw', [db.foods, db.entries, db.mealSets, db.recipes, db.planned, db.settings], async () => {
    await Promise.all([db.foods.clear(), db.entries.clear(), db.mealSets.clear(), db.recipes.clear(), db.planned.clear(), db.settings.clear()])
    await db.foods.bulkAdd(b.foods as never[])
    await db.entries.bulkAdd(b.entries as never[])
    // Backups made before saved meals or recipes existed simply don't have them.
    await db.mealSets.bulkAdd((b.mealSets ?? []) as never[])
    await db.recipes.bulkAdd((b.recipes ?? []) as never[])
    await db.planned.bulkAdd((b.planned ?? []) as never[])
